...
#[derive(Debug)]
pub struct Collection<S: Source<T>, T: Float, const N: usize> {
    pose: Pose<T>,
    children: [S; N],
    offsets: [Pose<T>; N],
}

impl<S: Source<T>, T: Float, const N: usize> Collection<S, T, N> {
...
impl<S: Source<T>, T: Float, const N: usize> Source<T> for Collection<S, T, N> {
    impl_pose_method!();
}

impl<S: Source<T> + Default, T: Float, const N: usize> Default for Collection<S, T, N> {
    fn default() -> Self {
        Self {
            pose: Pose::default(),
...
impl<S: Source<T>, T: Float, const N: usize> Field<T> for Collection<S, T, N> {
    #[inline]
